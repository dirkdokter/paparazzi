...
static volatile bool_t qr_code_spi_data_available = TRUE;

uint8_t testDataOut[4] = {1,2,3,4};
...
void qr_code_spi_link_init(void) {
  ...
  qr_code_spi_link_transaction.output_length = sizeof(testDataOut);
  qr_code_spi_link_transaction.output_buf    = testDataOut;
  qr_code_spi_link_transaction.input_length  = sizeof(testDataIn);
...
void qr_code_spi_link_periodic(void) {
  if (qr_code_spi_data_available) {
      ...
      spi_submit(&spi1, &qr_code_spi_link_transaction);
  }
}
