...
float force_allocation_fixedlift_device_max_climb         = FORCE_ALLOCATION_MAX_CLIMB;           // m/s
float force_allocation_fixedlift_device_pitch_of_vz       = FORCE_ALLOCATION_PITCH_OF_VZ;
float force_allocation_fixedlift_device_throttle_of_vz    = FORCE_ALLOCATION_THROTTLE_OF_VZ;
float force_allocation_fixedlift_device_pitch_trim        = FORCE_ALLOCATION_PITCH_TRIM;          // radians
float force_allocation_fixedlift_device_throttle_of_xdd   = FORCE_ALLOCATION_THROTTLE_OF_XDD;
float force_allocation_fixedlift_device_yawrate_of_ydd    = FORCE_ALLOCATION_YAWRATE_OF_YDD;
...
/* FIXME
  ...
  for (int i=0; i < LIFT_DEVICES_NB; i++)
  {
    ...
    else
    {
      ...
      float climb_speed = ((guidance_v_throttle_command - (MAX_PPRZ / 2)) * 2 * force_allocation_fixedlift_device_max_climb);  // MAX_PPRZ
      ...
      lift_device->commands[COMMAND_THRUST]  = (guidance_v_nominal_throttle)
                                      + climb_speed * force_allocation_fixedlift_device_throttle_of_vz
                                      + (-(stab_att_sp_euler.theta * MAX_PPRZ / 2) >> INT32_ANGLE_FRAC ) * force_allocation_fixedlift_device_throttle_of_xdd; // MAX_PPRZ
                                      //+ ((stab_att_sp_euler.theta * MAX_PPRZ) >> INT32_ANGLE_FRAC ); // MAX_PPRZ

      lift_device->commands[COMMAND_PITCH]   = ANGLE_BFP_OF_REAL(force_allocation_fixedlift_device_pitch_trim + climb_speed * force_allocation_fixedlift_device_pitch_of_vz / MAX_PPRZ);

      // Coordinated Turn
#if !defined(FREE_FLOATING_HEADING)
#pragma message "COORDINATED TURN"
      const int loop_rate = 512;
      lift_device->commands[COMMAND_YAW]    = stab_att_sp_euler.psi + lift_device->commands[COMMAND_ROLL] * force_allocation_fixedlift_device_yawrate_of_ydd / loop_rate;
#elif defined(QUADROTOR_HEADING)
      lift_device->commands[COMMAND_YAW]    =  stab_att_sp_euler.psi;

      lift_device->commands[COMMAND_YAW]    = ahrs.ltp_to_lift_euler.psi;
#endif
